...
var DWZ = {
	...
	init:function(pageFrag, options){
		...
		this._set.hasTitle = op.hasTitle;
	...
	$.fn.jBar = function(options){
		...
		return this.each(function(){
			...
			$(op.toggleBut, bar).click(function(){
                ...
                //hizo-start
				$(op.container).animate({left: cleft,width: cwidth},50-50,function(){
					bar.animate({left: barleft}, 500-500, function(){
						bar.hide();
						sbar.show().css("left", -50).animate({left: 5}, 200-200);
				...
				$(op.collapse,sbar).click(function(){
					...
					if(bar.is(":hidden")) {
						...
						bar.show().animate({left: sbarwidth}, 500-500);
			...
			$(op.toggleBut, sbar).click(function(){
				DWZ.ui.sbar = true;
				sbar.animate({left: -25}, 200-200, function(){				
					bar.show();
				});
				bar.animate({left: 5}, 800-800, function(){
					$(op.splitBar).show();
						...
						if(checkFn && $.isFunction(checkFn)) {
							$("div.ckbox", $this).each(function(){
								...
								ckbox.click(function(){
									...
									var items = [];
									if(true){
										var tnode = $(ckbox).parents('.treeCheck');
										var boxes = $(".checked input,.indeterminate input", tnode);//hizo -- $(".checked input", tnode);
										if(boxes.size() > 1) {
											$(boxes).each(function(){
									...
									checkFn({checked:checked, items:items ,tar:$(ckbox)});														
								});
							});
						}
					}
...
function initUI(_box){
	...
	if ($.fn.xheditor) {
		$("textarea.editor", $p).each(function(){
			...
			var upAttrs = [
				["upLinkUrl","upLinkExt","zip,rar,txt,doc"],
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external,ishidebar:$this.data('hidebar')});
		...
		return this.each(function(){
			...
			$box.find("li>a").click(function(){
				...
				if($a.data("hidebar")){
		            ...
		            navTab.openTab(tabid, url,{title:title, fresh:fresh, external:external,ishidebar:false});//toLoad
					...
					$.get($a.attr("href"), function(html){
                        $('.toggleCollapse div' ,$('#sidebar_s')).trigger('click');//show leftbar
                        $('.toggleCollapse div').show();
...
var navTab = {
	...
	closeAllTab: function(){
		if(DWZ._set.hasTitle){
			this._getTabs().remove();
			this._getPanels().trigger(DWZ.eventType.pageClear).remove();
...
(function($){
	...
	$.checkbox = {
		...
		select: function(_name, _type, _parent){
			...
			$checkboxLi = $parent.find(":checkbox[name='"+_name+"']:enabled");
			switch(_type){
				case "invert":
