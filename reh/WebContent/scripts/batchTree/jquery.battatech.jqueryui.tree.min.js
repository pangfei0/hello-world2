...
(function ($) {
    ...
    var $defaults = {
        ...
        , onselectednode: function (id, node, sender, span) { }
    ...
    $.fn.btechcotree = function (options) {
        ...
        function GetNodeObjectFromJson(jsonnode) {
            return {
                id: jsonnode[$settings.idTag]
                    , name: jsonnode[$settings.nameTag]
                    , parentid: jsonnode[$settings.parentidTag]
                    , href: jsonnode.href
                    ...
                    , data: typeof($settings.dataTag)==='string'?{'data':jsonnode[$settings.dataTag]}:{'fatherid':jsonnode[$settings.dataTag[0]],
                    	'level':jsonnode[$settings.dataTag[1]]}//{'communityAddr':jsonnode.communityAddr}//jsonnode.data //{'communityAddr':communityAddr}
                    , childnodes: []
            };
        }
        ...
        function CreateNode(node) {
            ...
            if (node.data && null != node.data) {
                for (var key in node.data) {
                    data_str += ' data-' + key + '="' + node.data[key] + '"';
                }
                data_str += " data-id='" + node.id + "'";
            }

            displaytree += "<li class='child-node' nodeid='" + node.id + "' " + data_str + ">";
            displaytree += "<div>";
        ...
        $("li div span[data-action='text']").bind("click", function (e) {
            $settings.onselectednode($(this).parent("div").parent("li").attr("nodeid"), $(this).parent("div").parent("li"), e, this);
    ...
    $.fn.btechcotree.ExpandAll = function () {
        $.fn.btechcotree.ExpandCollapseTree($("span." + $settings.class_node_collapse + ",#" + $settings.containerid + " > ul"), true);
    };
    ...
    //Get data of selected items what type is $type
    $.fn.btechcotree.SelectedData = function (sourcetree) {
        var tree = sourcetree||$("#" + $settings.containerid);
        var all = tree.find(":checked").parents('li.child-node');
        return all.map(function (i, e) {
            {
                //$(e).data().data = $(e).data();
                $(e).data().parentid = $(e).closest('ul').prev().data("id") || 0;
                $(e).data().name = $("div span", $(e)).text();
                $(e).data().level = $(e).data('data');
                //$(e).data().data = [];
                //$(e).data().data.type = type;
                return $(e).data();
    ...
    $.fn.btechcotree.GetSelectedData = function (type, sourcetree) {
        var tree = $(sourcetree);
        switch (type) {
            case 0: set = []; break;
            case 1: set = [].concat(tree.btechcotree.SelectedData(0,tree)).concat(tree.btechcotree.SelectedData(1,tree)); break;
            case 3: set = []; break;
            case 2: set = [].concat(tree.btechcotree.SelectedData(0,tree)).concat(tree.btechcotree.SelectedData(1,tree)).concat(tree.btechcotree.SelectedData(3,tree)).concat(tree.btechcotree.SelectedData(2,tree)); break;
            default: break;
        }
        return { root: set };
    };
    //Set selected items by classes IDs
    $.fn.btechcotree.SetItems = function (obj) {
        var tree = $("#" + $settings.containerid);
        $(obj.split(',')).each(function (i, e) { tree.find('li[nodeid~=' + e + '] div input').eq(0).trigger('click') });
        return obj;
    };
})(jQuery);